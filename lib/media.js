...
async function getArticleByQuestion(question) {
    const page = await browser.newPage();
    await page.goto(`https://google.ca/`);
    await page.type("body > div.L3eUgb > div.o3j99.ikrT4e.om7nvf > form > div:nth-child(1) > div.A8SBwf > div.RNNXgb > div > div.a4bIc > input", question);
    await (await page.$('body > div.L3eUgb > div.o3j99.ikrT4e.om7nvf > form > div:nth-child(1) > div.A8SBwf > div.RNNXgb > div > div.a4bIc > input')).press('Enter');
    await page.waitFor("#search");
    await page.click("#search > div > div > div a");
    let text;
    try { text = await page.$eval("article", (el) => (el.innerText)); } catch(e) {}
    if (!text) try { text = await page.$eval("main", (el) => (el.innerText)); } catch(e) {}
    if (!text) try { text = await page.$eval("#container", (el) => (el.innerText)); } catch(e) {}
    if (!text) try { text = await page.$eval("body", (el) => (el.innerText)); } catch(e) {}
    return text;
}
